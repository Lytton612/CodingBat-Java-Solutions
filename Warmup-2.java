...
int countXX(String str) {
  ...
  for(int i =0;i<str.length()-1;i++){
    if(str.substring(i,i+2).equals("xx")) amount++;
  }
  return amount;
}
...
boolean doubleX(String str) {
  ...
  if(pos!=str.length()-1 && str.charAt(pos+1)=='x') return true;
  return false;
}
...
public int last2(String str) {
  ...
  for(int i =0;i<str.length()-2;i++){
    if (str.substring(i,i+2).equals(key)) amount++;
  }
  return amount;
}
...
public boolean arrayFront9(int[] nums) {
  int to = 4;
  if(nums.length<4) to = nums.length;
  for(int i = 0;i<to;i++){
...
public int stringMatch(String a, String b) {
  ...
  for(int i =0;i<len-1;i++){
    if(a.substring(i,i+2).equals(b.substring(i,i+2))) amount++;
  }
  return amount;
}
...
public String altPairs(String str) {
  ...
  for(int i=0;i<str.length();i+=4){
    if(i+2>str.length()) out+=str.substring(i);
    else out+=str.substring(i,i+2);
  }
  return out;
}
...
public int array667(int[] nums) {
  ...
  for(int i =0;i<nums.length-1;i++){
	  if(nums[i]==6 && (nums[i+1]==6 || nums[i+1]==7)) amount++;
  }
  return amount;
}
